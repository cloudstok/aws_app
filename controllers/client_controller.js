...
async function myAllClient(req, res) {
    try {
        const [client] = await write.query(SQL_ALL_CLIENT);
        let myclient = []
        for (let x of client) {
            let check = x.assign_to.find(e=> e.value.id == res.locals.auth.id)
            if(check){
                delete x.assign_to
                delete x.is_deleted
                myclient.push(x)
        ...
        return res.status(200).send({ message: "User list", data: myclient });
    }
    catch (err) {
        console.log(err)
        return res.status(400).json({ status: false, message: "Something went wrong" })
...
module.exports = { findAllClients, insertClient, findClientById, myAllClient }